...
from envs import optim_cache_environment
...
def evaluate_policy(args, env, agent, state_norm):
    ...
    for _ in range(times):
        ...
        while not done:
            ...
            action = a  # 找出具体的计算节点和缓存节点
            if action == 0:
                episode_cloud_count += 1
            elif action < rsu_number:
                episode_rsu_offloading_count += 1
            elif action < (rsu_number + vehicle_number):
...
def train(args):
    ...
    env = optim_cache_environment.RoadState()
    env_evaluate = optim_cache_environment.RoadState()  # When evaluating the policy, we need to rebuild an environment
