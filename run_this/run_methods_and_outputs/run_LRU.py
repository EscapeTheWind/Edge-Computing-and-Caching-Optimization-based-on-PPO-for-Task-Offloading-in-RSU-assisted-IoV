...
from envs import LRU_environment
...
def evaluate_policy(args, env, agent, state_norm):
    ...
    return evaluate_reward / times, evaluate_cur_hitrate / times, (
                evaluate_all_hitrate / times) / 100, evaluate_time / times, (
                       evaluate_none_cache / times) / 100, evaluate_rsu_offloading_count / times, evaluate_vehicle_count / times, evaluate_cloud_count / times
...
def train(args):
    ...
    env = LRU_environment.RoadState()
    env_evaluate = LRU_environment.RoadState()  # When evaluating the policy, we need to rebuild an environment
    ...
    while total_steps < args.max_train_steps:
        ...
        while not done:
            ...
            if total_steps % args.evaluate_freq == 0:
                evaluate_num += 1
                evaluate_reward, evaluate_cur_hitrate, evaluate_all_hitrate, evaluate_time, evaluate_none_cache, evaluate_rsu_offloading_count, evaluate_vehicle_offloading_count, evaluate_cloud_offloading_count = evaluate_policy(
                ...
                if ma_rewards:
                    ma_rewards.append(0.75 * ma_rewards[-1] + 0.25 * evaluate_reward)
                else:
                ...
                if ma_time_:
                    ma_time_.append(0.75 * ma_time_[-1] + 0.25 * evaluate_time)
                else:
