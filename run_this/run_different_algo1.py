...
from run_this.run_methods_and_outputs.run_LFU import *
from run_this.run_methods_and_outputs.run_LRU import *
from utils import *
...
reward1, ma_hit_rate1, time1, ma_all_hit_rate1, args1 = run_lfu();
# 保存all hit ratio曲线
with open("./data/all_hitrate.txt", "a") as file:
    file.write(str(ma_all_hit_rate1) + "\n")

reward2, ma_hit_rate2, time2, ma_all_hit_rate2, args2 = run_lru();
# 保存all hit ratio曲线
with open("./data/all_hitrate.txt", "a") as file:
...
if len(lines) == 3:
    plot_hit_rate_with_different_algo(args, tag='train')
else:
