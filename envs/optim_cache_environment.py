...
from envs import optim_cache_config
...
class RoadState(gym.Env):
    ...
    def __init__(
            self,
            env_config: Optional[optim_cache_config.VehicularEnvConfig] = None,
        ...
        if env_config is None:
            self._config = optim_cache_config.VehicularEnvConfig()
        else:
    ...
    def _function_cache_allocation(self, action) -> None:
        """放置缓存任务"""
        action_arr = [action, 2]  # 找出具体的计算节点和缓存节点
        ...
        for rsu in self._rsu_list.get_rsu_list():
            ...
            i += 1
        re = self._rsu_list.get_rsu_list()[action_arr[1]].store_task_with_popularity(
            self._function.get_output_size(),
            self._function.get_kind(),
            self._function.get_deadline(),
        ...
        self._cache_matrix.update_cache_status_by_one(action_arr[1], self._function.get_kind() - 1)
        if re:  # 存入任务时如果因空间不足删除了其他任务，将其他任务的缓存情况置0
            for task_info in re:
                self._rsu_list.get_rsu_list()[action_arr[1]].delete_task(task_info)
                self._cache_matrix.update_cache_status_by_zero(action_arr[1], task_info - 1)
    ...
    def _function_compute_allocation(self, action) -> None:
        ...
        if action_arr[0] == 0:
            ...
            self._rsu_list.get_rsu_list()[action_arr[0] - 1].get_task_list().add_task_list(
                self._function.get_input_size())
        elif action_arr[0] < (4 + self._vehicle_list.get_vehicle_number()):
            self._vehicle_list.get_vehicle_list()[action_arr[0] - 4].get_task_list().add_task_list(
                self._function.get_input_size())
        else:
            pass
    ...
    def get_time_reward(self, action):
        ...
        if action_arr[0] == 0:  # 卸载到Cloud
            ...
            wait_time = self.state[action_arr[0] + 6] / self._rsu_list.get_rsu_list()[
                action_arr[0] - 1].get_rsu_compute_speed()
            compute_time = self.state[0] / (
                    self.state[1] * self._rsu_list.get_rsu_list()[action_arr[0] - 1].get_rsu_compute_speed())
            ...
            wait_time = self.state[action_arr[0] + 6] / self._vehicle_list.get_vehicle_list()[
                action_arr[0] - 4].get_vehicle_compute_speed()
            compute_time = self.state[0] / (
